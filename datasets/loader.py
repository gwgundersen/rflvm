...
def load_bball(metric_list, model, exposure_list, age = None, gaussian_indices = None, poisson_indices = None,
    ...
    if model == "mixed":
        metric_df = df[(df["age"] == age) & (df["minutes"] > 0)][metric_list + ["id"]]
        exposure_df = df[(df["age"] == age) & (df["minutes"] > 0)][exposure_list + ["id"]]
        names_df = df[(df["age"] == age) & (df["minutes"] > 0)][["id","name"]].drop_duplicates()["name"].values
        player_id_df = df[(df["age"] == age) & (df["minutes"] > 0)][["id"]].drop_duplicates()
